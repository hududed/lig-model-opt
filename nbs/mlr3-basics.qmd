...
This notebook runs benchmarks on three different datasets `GOQ`, `PI`, `GOPI` from <https://github.com/hududed/lig-model-opt> using the **mlr3** packages. We first load the data and preview the table.

```{r}
# d.full <- read.csv("GOQ.csv", stringsAsFactors = TRUE) # init with full data
# d.goq <- d.full$select= -c(campaign, initial) # keep only these features
setwd("..")
...
Let's create our first task. Here we can define our inputs `backend` and objective `target`. This task specializes `Task` and `TaskSupervised` for regression problems. The target column is assumed to be numeric. The `task_type` is set to "regr".
...
As we see there's about 115 different learner objects. Let's dive in one of them:
...
What you might've caught is that we have also made the `Learner` object that we will train using random forest!
...
Now we see that the model has been trained using random forest `ranger`, and the results are slightly improved.
Technically we can already predict something! Lets have a look:
